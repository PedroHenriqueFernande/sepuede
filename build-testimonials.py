...
for slug, name, role, title, quote, position in people:
    ...
    cards.append(f'<figure class="testimonial">{rating_html}<blockquote><strong>{html.escape(title)}</strong>{html.escape(quote)}</blockquote><figcaption><img src="assets/testimonials/{slug}.png" alt="Retrato generado por IA de {name}, personaje ficticio" width="68" height="68" loading="lazy" decoding="async" style="object-position:{position}"/><div><strong>{name}</strong><span class="testimonial__role">{role}</span></div></figcaption></figure>')
content = '<div class="testimonials__inner"><header class="testimonials__header"><span class="testimonials__eyebrow">Para el aula y el hogar</span><h2 id="testimonios-title">Convivir también se aprende <span>jugando</span></h2><p class="testimonials__intro">Cuatro miradas sobre cómo aprovechar ¿Se Puede o No Se Puede? en el día a día.</p></header><div class="testimonials__grid">' + ''.join(cards) + '</div><p class="testimonials__notice">Testimonios ficticios con fines ilustrativos. Los nombres y los retratos generados por IA no corresponden a clientes reales.</p></div>'
